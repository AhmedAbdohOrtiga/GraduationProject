...
SVM_Node::SVM_Node(Node* _left, Node* _right, long long _num) {
	...
	Mat* r = new Mat();
	Mat* rr = new Mat();
	svm.train(*data, *labels, *r, *rr, CvSVMParams());
...
//	float* vecs = new float[sv_count * feature_size]; //(malloc(* sizeof(float)));
	vector<float> vecs; //[sv_count * feature_size]; //(malloc(* sizeof(float)));

	for (unsigned int i = 0; i < sv_count; ++i) {
		const float* vec = svm.get_support_vector(i);
		for (int j = 0; j < feature_size; ++j) {
			vecs.push_back(vec[j]); //vec[j];
		}
	}

	float* vecss = &vecs[0];
	Mat* vectors = new Mat(sv_count, feature_size, data_type, vecss);
